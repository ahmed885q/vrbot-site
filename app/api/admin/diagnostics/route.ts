...
export async function GET() {
  ...
  try { const r = await fetch(process.env.WS_HUB_HEALTH_URL || "http://65.109.214.187:8080/api/status", { signal: AbortSignal.timeout(15000) }); hubHealth = await r.json(); } catch (e: any) { hubHealth = { error: e.message, status: "offline" }; }
