...
type Farm = { id: string; name: string; server: string | null; notes: string | null; created_at: string; bot_enabled?: boolean; bot_status?: string; cloud_status?: string; cloud_farm_id?: number; cloud?: { farm_id: number; status: string; online: boolean } | null };
...
const tx: Record<Lang, Record<string, string>> = {
  ...
  en: {
    ...
    cancel: "Cancel", create: "Create", creating: "Creating...", deleteFarm: "Delete", deleteConfirm: "Delete this farm?",
    loading: "Loading...", logout: "Logout", selectAll: "Select All", deselectAll: "Deselect All",
    runSelected: "Run Selected", stopAll: "Stop All", selected: "Selected", online: "Online", offline: "Offline",
...
export default function DashboardClient() {
  ...
  useEffect(() => {
    try { const sv = localStorage.getItem("vrbot_lang") as Lang; if (sv && tx[sv]) setLang(sv); } catch {}
    setMounted(true);
  }, []);
  ...
  const loadTokens = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (d.tokens_total === 0 && !d.trial_granted) {
          const tr = await fetch("/api/tokens/grant-trial", { method: "POST" });
          if (tr.ok) { const r2 = await fetch("/api/tokens/status"); if (r2.ok) { setTokens(await r2.json()); setLoadingTokens(false); return; } }
        }
        setTokens(d);
      }
    } catch {}
  ...
  // Load farms with cloud status merged
  const loadFarms = useCallback(async () => {
    setLoadingFarms(true);
    try {
      // Try cloud-merged endpoint first
      const cloudRes = await fetch("/api/cloud/farms");
      if (cloudRes.ok) {
        const d = await cloudRes.json();
        const fl = d.farms || [];
        setFarms(fl);
        setCloudOnline(d.cloud_online || false);
        if (fl.length > 0 && !selectedFarmId) setSelectedFarmId(fl[0].id);
        setLoadingFarms(false);
        return;
      }
    } catch {}

    // Fallback to regular farms list
    try {
  ...
  const loadCloudStatus = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const d = await res.json();
        setCloudStatus(d);
        setCloudOnline(d.ok && d.running !== undefined);
      } else {
  ...
  useEffect(() => {
    if (user) { loadTokens(); loadFarms(); loadCloudStatus(); }
  }, [user, loadTokens, loadFarms, loadCloudStatus]);

  // Auto-refresh cloud status every 30 seconds
  useEffect(() => {
    if (!user) return;
    const interval = setInterval(() => { loadCloudStatus(); }, 30000);
    return () => clearInterval(interval);
  }, [user, loadCloudStatus]);
  ...
  async function handleDeleteFarm(farmId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCheckedFarms(new Set(checkedFarms));
        loadFarms(); loadTokens();
      }
    } catch {}
  }

  async function handleRetryCloud(farmId: string) {
    try {
  ...
  function cloudStatusColor(cs?: string) {
    if (!cs) return "#64748b";
    if (cs === "active") return "#10b981";
    if (cs === "provisioning") return "#f59e0b";
    if (cs === "cloud_error") return "#ef4444";
    return "#64748b";
  }
  function cloudStatusLabel(cs?: string) {
    if (!cs || cs === "local") return s.local;
    if (cs === "active") return s.cloudActive;
    if (cs === "provisioning") return s.provisioning;
    if (cs === "cloud_error") return s.cloudError;
    if (cs === "pending") return s.provisioning;
    return cs;
  }

  const getLogColor = (m: HubMessage) => {
    switch (m.type) {
      case "task_complete": return "#34d399";
      case "task_error": case "error": return "#f87171";
  ...
  const cloudFarmsCount = farms.filter(f => f.cloud_status === "active" || f.cloud?.online).length;
    ...
    <div dir={isRtl ? "rtl" : "ltr"} style={{ minHeight: "100vh", background: "#0b0f1a", fontFamily: "Segoe UI, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0f1629, #1a1145)", borderBottom: "1px solid rgba(139,92,246,0.15)", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 10px", background: cloudOnline ? "rgba(16,185,129,0.08)" : "rgba(239,68,68,0.08)", border: `1px solid ${cloudOnline ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.2)"}`, borderRadius: 8 }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%", background: cloudOnline ? "#10b981" : "#ef4444" }} />
            <span style={{ fontSize: 11, color: cloudOnline ? "#10b981" : "#ef4444", fontWeight: 600 }}>
              {cloudOnline ? s.cloudOnline : s.cloudOffline}
            </span>
          </div>
          <a href="/dashboard/farms" style={{ padding: "6px 14px", background: "rgba(240,165,0,0.1)", border: "1px solid rgba(240,165,0,0.3)", borderRadius: 8, color: "#f0a500", textDecoration: "none", fontSize: 12, fontWeight: 600 }}>مزارعي</a>
        ...
        : farms.length === 0 ? (
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr>{["", "Name", "Server", "Status", "Cloud", "Task", "Created", ""].map((h, i) => <th key={i} style={{ padding: "10px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)", textAlign: "left", fontSize: 11, color: "rgba(255,255,255,0.35)", fontWeight: 600 }}>{h}</th>)}</tr></thead>
              <tbody>{filteredFarms.map(f => {
                const st = getFarmStatus(f.id);
                const isSel = f.id === selectedFarmId;
                const isChk = checkedFarms.has(f.id);
                return (
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(auto-fill, minmax(${farms.length > 20 ? "200px" : "300px"}, 1fr))`, gap: 10 }}>
            {filteredFarms.map((f, i) => {
              ...
              const isChk = checkedFarms.has(f.id);
              return (
                <div key={f.id} onClick={() => setSelectedFarmId(f.id)} style={{ background: isSel ? "rgba(139,92,246,0.08)" : isChk ? "rgba(139,92,246,0.04)" : "rgba(255,255,255,0.03)", borderRadius: 12, padding: farms.length > 20 ? "12px" : "16px", border: isSel ? "2px solid rgba(139,92,246,0.5)" : isChk ? "1px solid rgba(139,92,246,0.3)" : "1px solid rgba(255,255,255,0.06)", cursor: "pointer", transition: "all 0.2s", position: "relative", animation: `fadeIn ${0.2 + (i % 20) * 0.03}s ease` }}>
                  {/* Checkbox */}
