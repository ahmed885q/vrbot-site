...
import { usePagination, Pager } from '../components/Pagination'
...
export default function DiagnosticsPage() {
  ...
  // Pagination hooks
  const farmsPager = usePagination(data?.farms ?? [])
  const tokensPager = usePagination(data?.tokens ?? [])
  const subsPager = usePagination(data?.subscriptions ?? [])
  const keysPager = usePagination(filteredKeys)
  const usersPager = usePagination(data?.users ?? [])
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ ...S.card, borderTopLeftRadius: 0, marginTop: 0, overflowX: 'auto' }}>
        ...
        {tab === 'tokens' && (<div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['Email','Total','Used','Remaining','Trial','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead><tbody>{tokensPager.paged.map((tk: any) => (<tr key={tk.user_id}><td style={S.td}><span style={{ fontSize: '12px' }}>{tk.email}</span></td><td style={{ ...S.td, fontWeight: 700, color: '#3b82f6' }}>{tk.tokens_total}</td><td style={{ ...S.td, fontWeight: 700, color: '#f59e0b' }}>{tk.tokens_used}</td><td style={{ ...S.td, fontWeight: 700, color: '#22c55e' }}>{tk.tokens_total - tk.tokens_used}</td><td style={S.td}><span style={tk.trial_granted ? S.badge('#f59e0b','#2a1a00') : S.badge('#888','#1a1a2a')}>{tk.trial_granted ? 'Yes' : 'No'}</span></td><td style={S.td}><button style={S.btn('#f59e0b','#000')} onClick={() => doAction('reset_tokens', { userId: tk.user_id })}>Reset</button></td></tr>))}</tbody></table>
        ...
        {tab === 'subs' && (<div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['Email','Plan','Status','Expires','Source','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead><tbody>{subsPager.paged.map((sub: any) => {const on = sub.status === 'active'; const src = sub.stripe_customer_id === 'admin_manual' ? 'Manual' : sub.pro_key_code ? 'Pro Key' : sub.stripe_customer_id?.startsWith('cus_') ? 'Stripe' : 'PayPal'; return (<tr key={sub.id}><td style={S.td}><span style={{ fontSize: '12px' }}>{sub.email}</span></td><td style={{ ...S.td, fontWeight: 600 }}>{sub.plan || '-'}</td><td style={S.td}><span style={on ? S.badge('#22c55e','#052e16') : S.badge('#ef4444','#2a0a0a')}>{sub.status}</span></td><td style={{ ...S.td, fontSize: '12px' }}>{shortDate(sub.current_period_end)}</td><td style={S.td}><span style={S.badge('#06b6d4','#0a2a3a')}>{src}</span></td><td style={S.td}><div style={{ display: 'flex', gap: '4px' }}>{on ? <button style={S.btn('#ef4444','#fff')} onClick={() => doAction('deactivate_subscription', { userId: sub.user_id })}>Deactivate</button> : <button style={S.btn('#22c55e','#fff')} onClick={() => doAction('activate_subscription', { userId: sub.user_id })}>Activate</button>}</div></td></tr>)})}</tbody></table>
        ...
        {tab === 'proKeys' && (<div>
          ...
          </tr></thead><tbody>{keysPager.paged.map((k: any) => (<tr key={k.id} style={{ background: selectedKeys.includes(k.id) ? '#1a0a2e' : 'transparent' }}>
        ...
        {tab === 'users' && (<div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['Email','Created','Last Login','Status','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead><tbody>{usersPager.paged.map((u: any) => (<tr key={u.id}><td style={S.td}>{u.email}</td><td style={{ ...S.td, fontSize: '12px', color: '#888' }}>{shortDate(u.created_at)}</td><td style={{ ...S.td, fontSize: '12px', color: '#888' }}>{timeAgo(u.last_sign_in)}</td><td style={S.td}><span style={u.banned ? S.badge('#ef4444','#2a0a0a') : S.badge('#22c55e','#052e16')}>{u.banned ? 'Banned' : 'Active'}</span></td><td style={S.td}><div style={{ display: 'flex', gap: '4px' }}>{u.banned ? <button style={S.btn('#22c55e','#fff')} onClick={() => doAction('unban_user', { userId: u.id })}>Unban</button> : <button style={S.btn('#f59e0b','#000')} onClick={() => doAction('ban_user', { userId: u.id })}>Ban</button>}<button style={S.btn('#ef4444','#fff')} onClick={() => { if(confirm('DELETE ' + u.email + '?')) doAction('delete_user', { userId: u.id }) }}>Delete</button></div></td></tr>))}</tbody></table>
