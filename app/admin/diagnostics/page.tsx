  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '16px' }}>
        {[
          { v: stats.totalUsers, l: 'Users', c: '#3b82f6' }, { v: stats.totalFarms, l: 'Farms', c: '#8b5cf6' },
          { v: stats.activeFarms, l: 'Active', c: '#22c55e' }, { v: stats.activeSubs, l: 'Subs', c: '#f59e0b' },
          { v: (stats.totalTokensUsed||0)+'/'+(stats.totalTokensAvail||0), l: 'Tokens', c: '#06b6d4' },
          { v: stats.totalKeys, l: 'Keys', c: '#a855f7' }, { v: stats.usedKeys, l: 'Used', c: '#ec4899' },
        ].map((s, i) => (<div key={i} style={S.stat(s.c)}><div style={{ fontSize: '24px', fontWeight: 800, color: s.c }}>{s.v}</div><div style={{ fontSize: '11px', color: '#888' }}>{s.l}</div></div>))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
        ...
        ].map(t => (<div key={t.k} style={S.tabS(tab === t.k)} onClick={() => setTab(t.k)}>{t.l}{t.n !== null ? ` (${t.n ?? 0})` : ''}</div>))}
      </div>
      ...
      <div style={{ ...S.card, borderTopLeftRadius: 0, marginTop: 0, overflowX: 'auto' }}>
        ...
        {tab === 'farms' && (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>{['Email','Name','Server','Bot','Status','Activity','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead>
            <tbody>{(data?.farms ?? []).map((f: any) => (
              <tr key={f.id}>
                <td style={S.td}><span style={{ fontSize: '12px' }}>{f.email}</span></td>
                <td style={{ ...S.td, fontWeight: 600 }}>{f.name}</td>
                <td style={S.td}>{f.server || '-'}</td>
                <td style={S.td}><span style={f.bot_enabled ? S.badge('#22c55e','#052e16') : S.badge('#888','#1a1a2a')}>{f.bot_enabled ? 'ON' : 'OFF'}</span></td>
                ...
                <td style={S.td}><div style={{ display: 'flex', gap: '4px' }}>
                  ...
                  <button style={S.btn('#ef4444','#fff')} onClick={() => { if(confirm('Delete farm + refund token?')) doAction('delete_farm', { farmId: f.id }) }}>Delete</button>
                </div></td>
              </tr>))}</tbody>
          </table>
        )}
        ...
        {tab === 'tokens' && (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>{(data?.tokens ?? []).map((t: any) => (
              <tr key={t.user_id}>
                <td style={S.td}><span style={{ fontSize: '12px' }}>{t.email}</span></td>
                <td style={{ ...S.td, fontWeight: 700, color: '#3b82f6' }}>{t.tokens_total}</td>
                <td style={{ ...S.td, fontWeight: 700, color: '#f59e0b' }}>{t.tokens_used}</td>
                <td style={{ ...S.td, fontWeight: 700, color: '#22c55e' }}>{t.tokens_total - t.tokens_used}</td>
                <td style={S.td}><span style={t.trial_granted ? S.badge('#f59e0b','#2a1a00') : S.badge('#888','#1a1a2a')}>{t.trial_granted ? 'Yes' : 'No'}</span></td>
                <td style={S.td}><button style={S.btn('#f59e0b','#000')} onClick={() => doAction('reset_tokens', { userId: t.user_id })}>Reset</button></td>
              </tr>))}</tbody>
          </table>
        )}
        ...
        {tab === 'subs' && (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>{(data?.subscriptions ?? []).map((s: any) => {
              ...
              const src = s.stripe_customer_id === 'admin_manual' ? 'Manual' : s.pro_key_code ? 'Pro Key' : s.stripe_customer_id?.startsWith('cus_') ? 'Stripe' : 'PayPal'
              return (<tr key={s.id}>
                <td style={S.td}><span style={{ fontSize: '12px' }}>{s.email}</span></td>
                <td style={{ ...S.td, fontWeight: 600 }}>{s.plan || '-'}</td>
        ...
        {tab === 'proKeys' && (
          <div>
            ...
            {genCodes.length > 0 && (
              <div style={{ ...S.card, background: '#1a0a2e', borderColor: '#a855f740', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span style={{ color: '#a855f7', fontWeight: 700 }}>Generated {genCodes.length} keys:</span>
                  <button style={S.btn('#a855f7','#fff')} onClick={() => copyText(genCodes.join('\n'))}>Copy All</button>
                </div>
                <div style={{ maxHeight: '150px', overflow: 'auto', fontSize: '12px', fontFamily: 'monospace', color: '#ccc' }}>{genCodes.map((c,i) => <div key={i}>{c}</div>)}</div>
              </div>
            )}
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>{['Code','Status','Used By','Batch','Note','Created','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>{(data?.proKeys ?? []).map((k: any) => (
                <tr key={k.id}>
                  <td style={S.td}><span style={{ fontFamily: 'monospace', fontSize: '11px', cursor: 'pointer' }} onClick={() => copyText(k.code)} title="Click to copy">{k.code?.slice(0,8)}...</span></td>
                  ...
                  <td style={{ ...S.td, fontSize: '12px', maxWidth: '150px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{k.note || '-'}</td>
                  <td style={{ ...S.td, fontSize: '12px', color: '#888' }}>{shortDate(k.created_at)}</td>
                  <td style={S.td}>
                    {!k.revoked && !k.is_used && <button style={S.btn('#ef4444','#fff')} onClick={() => doAction('revoke_key', { keyId: k.id })}>Revoke</button>}
                    {k.revoked && <button style={S.btn('#22c55e','#fff')} onClick={() => doAction('unrevoke_key', { keyId: k.id })}>Unrevoke</button>}
                  </td>
                </tr>))}</tbody>
            </table>
          </div>
        ...
        {tab === 'users' && (
          <div>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center', padding: '12px', background: '#1a1a2e', borderRadius: '8px' }}>
              <span style={{ fontSize: '13px', color: '#888' }}>Create User:</span>
              <input value={newEmail} onChange={e => setNewEmail(e.target.value)} style={{ ...S.input, width: '220px' }} placeholder="email@example.com" />
              <input value={newPass} onChange={e => setNewPass(e.target.value)} style={{ ...S.input, width: '150px' }} placeholder="Password" type="password" />
              <button style={S.btn('#3b82f6','#fff')} onClick={() => { if(newEmail && newPass) { doAction('create_user', { email: newEmail, password: newPass }); setNewEmail(''); setNewPass('') } }}>Create</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>{['Email','Created','Last Login','Status','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>{(data?.users ?? []).map((u: any) => (
                <tr key={u.id}>
                  ...
                  <td style={S.td}><span style={u.banned ? S.badge('#ef4444','#2a0a0a') : S.badge('#22c55e','#052e16')}>{u.banned ? 'Banned' : 'Active'}</span></td>
                  <td style={S.td}><div style={{ display: 'flex', gap: '4px' }}>
                    ...
                    <button style={S.btn('#ef4444','#fff')} onClick={() => { if(confirm('DELETE user ' + u.email + ' + all data?')) doAction('delete_user', { userId: u.id }) }}>Delete</button>
                  </div></td>
                </tr>))}</tbody>
            </table>
          </div>
        ...
        {tab === 'protection' && (
          <div style={{ maxWidth: '500px' }}>
            <h3 style={{ margin: '0 0 20px', color: '#8b5cf6' }}>Protection Settings</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                <input type="checkbox" checked={pEnabled} onChange={e => setPEnabled(e.target.checked)} style={{ width: '18px', height: '18px' }} />
                <span style={{ fontSize: '14px' }}>Enabled</span>
              </label>
              <div>
                <label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '4px' }}>Sensitivity (1-5)</label>
                <input type="number" min={1} max={5} value={pSensitivity} onChange={e => setPSensitivity(Number(e.target.value))} style={{ ...S.input, width: '100%' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '4px' }}>Max Requests</label>
                <input type="number" min={1} value={pMaxReq} onChange={e => setPMaxReq(Number(e.target.value))} style={{ ...S.input, width: '100%' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '4px' }}>Block Duration (minutes)</label>
                <input type="number" min={1} value={pBlockMin} onChange={e => setPBlockMin(Number(e.target.value))} style={{ ...S.input, width: '100%' }} />
              </div>
              <button style={{ ...S.btn('#8b5cf6','#fff'), padding: '12px', fontSize: '14px' }} onClick={() => doAction('save_protection', { settings: { enabled: pEnabled, sensitivity: pSensitivity, max_requests: pMaxReq, block_duration_minutes: pBlockMin } })}>Save Settings</button>
            </div>
          </div>
        )}
      </div>
