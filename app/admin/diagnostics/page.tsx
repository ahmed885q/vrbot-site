  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '16px' }}>
        {[{ v: stats.totalUsers, l: 'Users', c: '#3b82f6' },{ v: stats.totalFarms, l: 'Farms', c: '#8b5cf6' },{ v: stats.activeFarms, l: 'Active', c: '#22c55e' },{ v: stats.activeSubs, l: 'Subs', c: '#f59e0b' },{ v: (stats.totalTokensUsed||0)+'/'+(stats.totalTokensAvail||0), l: 'Tokens', c: '#06b6d4' },{ v: stats.totalKeys, l: 'Keys', c: '#a855f7' },{ v: stats.usedKeys, l: 'Used', c: '#ec4899' }].map((s, i) => (<div key={i} style={S.stat(s.c)}><div style={{ fontSize: '24px', fontWeight: 800, color: s.c }}>{s.v}</div><div style={{ fontSize: '11px', color: '#888' }}>{s.l}</div></div>))}
      </div>
      ...
      <div style={{ ...S.card, borderTopLeftRadius: 0, marginTop: 0, overflowX: 'auto' }}>
        ...
        {tab === 'proKeys' && (<div><div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}><input type="number" min={1} max={100} value={keyCount} onChange={e => setKeyCount(Number(e.target.value))} style={{ ...S.input, width: '70px' }} placeholder="Count" /><input value={keyTag} onChange={e => setKeyTag(e.target.value)} style={{ ...S.input, width: '150px' }} placeholder="Batch tag" /><input value={keyNote} onChange={e => setKeyNote(e.target.value)} style={{ ...S.input, width: '200px' }} placeholder="Note" /><button style={S.btn('#a855f7','#fff')} onClick={() => doAction('generate_keys', { count: keyCount, batchTag: keyTag || null, note: keyNote || null })}>Generate</button></div>{genCodes.length > 0 && (<div style={{ ...S.card, background: '#1a0a2e', borderColor: '#a855f740', marginBottom: '16px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ color: '#a855f7', fontWeight: 700 }}>Generated {genCodes.length} keys:</span><button style={S.btn('#a855f7','#fff')} onClick={() => copyText(genCodes.join('\n'))}>Copy All</button></div><div style={{ maxHeight: '150px', overflow: 'auto', fontSize: '12px', fontFamily: 'monospace', color: '#ccc' }}>{genCodes.map((c,i) => <div key={i}>{c}</div>)}</div></div>)}<table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['Code','Status','Used By','Batch','Note','Created','Actions'].map((h,i) => <th key={i} style={S.th}>{h}</th>)}</tr></thead><tbody>{(data?.proKeys ?? []).map((k: any) => (<tr key={k.id}><td style={S.td}><span style={{ fontFamily: 'monospace', fontSize: '11px', cursor: 'pointer' }} onClick={() => copyText(k.code)} title="Click to copy">{k.code?.slice(0,8)}...</span></td><td style={S.td}><span style={k.revoked ? S.badge('#ef4444','#2a0a0a') : k.is_used ? S.badge('#f59e0b','#2a1a00') : S.badge('#22c55e','#052e16')}>{k.revoked ? 'Revoked' : k.is_used ? 'Used' : 'Available'}</span></td><td style={{ ...S.td, fontSize: '12px' }}>{k.usedByEmail || '-'}</td><td style={S.td}>{k.batch_tag || '-'}</td><td style={{ ...S.td, fontSize: '12px', maxWidth: '150px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{k.note || '-'}</td><td style={{ ...S.td, fontSize: '12px', color: '#888' }}>{shortDate(k.created_at)}</td><td style={S.td}>{!k.revoked && !k.is_used && <button style={S.btn('#ef4444','#fff')} onClick={() => doAction('revoke_key', { keyId: k.id })}>Revoke</button>}{k.revoked && <button style={S.btn('#22c55e','#fff')} onClick={() => doAction('unrevoke_key', { keyId: k.id })}>Unrevoke</button>}</td></tr>))}</tbody></table></div>)}
